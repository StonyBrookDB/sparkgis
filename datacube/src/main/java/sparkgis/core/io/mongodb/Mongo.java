...
public static BasicDBObject prepareQuery(final Map<String, Object> params){
        ...
        while(iterator.hasNext()) {
                ...
                else if(key.startsWith("features"))
                {
                        ...
                        Double val1 = Double.valueOf(         vals[0]  );
                        Double val2 = Double.valueOf(         vals[1]  );
                        System.out.println("loglolglog: "+   vals[0]  );
                        BasicDBObject tmp = new BasicDBObject().append("$gt", val1).append("$lte", val2);
                        query.append(key,tmp);

                        // DBObject query1 = query.append(  "features.Perimeter", new BasicDBObject().append("$gt", 120).append("$lte", 122)   )
                }
                else
                        query.append(key, (String)entry.getValue());
        }
