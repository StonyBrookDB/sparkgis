...
var queue = kue.createQueue({disableSearch : false});
...
// curl --form "image=@results.json" --form
// "algos=yi-algorithm-v1,yi-algorithm-v11" --form
// "caseids=TCGA-02-0001-01Z-00-DX1" --form "metric=jaccard" --form
// "result_exe_id=cheuk_testabc"
// http://localhost:8127/api/upload_and_generate_heatmap

router.post('/upload_and_generate_heatmap',
            function(req, res) { fun_upload_and_generate_heatmap(req, res); });

// curl -X POST --data
// "algos=yi-algorithm-v1,yi-algorithm-v11&caseids=TCGA-02-0001-01Z-00-DX1&metric=jaccard&input=mongodb&output=mongodb&inputdb=u24_segmentation&inputcollection=results&outputdb=u24_segmentation&outputcollection=results&result_exe_id=cheuk_testabc"
// http://localhost:8127/api/generate_heatmap

router.post('/generate_heatmap',
            function(req, res) { fun_generate_heatmap(req, res); });
...
var fun_upload_and_generate_heatmap = function(req, res) {
  ...
  form.parse(req, function(err, fields, files) {
    ...
    //    var params_array = ['algos', 'caseids', 'metric', 'input', 'output',
    //    'inputdb', 'inputcollection' , 'outputdb' ,'outputcollection'
    //    ,'result_exe_id']
    // var cmd_params = '';
    ...
    cmd_params = cmd_params + " --upload yes" +
                 " --uid " + uni_job_id
...
var fun_generate_heatmap =
  ...
  var params_array =
      [
        'algos', 'caseids', 'metric', 'input', 'output', 'inputdb',
        'inputcollection', 'outputdb', 'outputcollection', 'result_exe_id'
      ]
  ...
  var job = queue
    ...
    queue.process('upload_and_get_heatmap', 2, function(job, done) {
      ...
      var command1 = "mongoimport --host " + HOST + ":" + PORT + " --db " +
                     DBNAME + " --collection " + COLLECTION1 + " --file " +
                     tmp_path

      // var command1 ="mongoimport --host localhost:27017 --db u24_segmentation
      // --collection "+COLLECTION1+" --file " + tmp_path
...
var fun_get_heat_map =
  ...
  exec(SPARK_CMD_HEATMAP + cmd_params + log_cmd,
...
var update_status_by_jobid = function(
    status, jobid) { redis_client.set(jobid, status, redis.print); }

var update_job_status =
    function(job) {
  ...
  job.on('complete',
      ...
      .on('failed attempt',
          function(errorMessage, doneAttempts) {
      ...
      .on('progress', function(progress, data) {
        // console.log('\r  job #' + job.id + ' ' + progress + '% complete with
        // data ', data );
