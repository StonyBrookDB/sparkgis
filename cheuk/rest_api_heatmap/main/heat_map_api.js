...
queue.process('upload_and_get_heatmap', 2, function(job, done) {
  ...
  var command1 =
      "mongoimport --host localhost:27017 --db u24_segmentation --collection " +
      COLLECTION1 + " --file " + tmp_path
...
var fun_get_heat_map =
  ...
  var command1 =
      '/home/cochung/spark_full/spark-1.4.1/bin/spark-submit --class sparkgis.SparkGISMain /home/cochung/spark_full/spark-gis/cheuk/spark-gis-prod/spark-gis/target/uber-spark-gis-1.0.jar';
  ...
  exec(command1 + cmd_params + log_cmd, function(error, stdout, stderr) {
    ...
    app.post('/api/get_heat_map', function(req, res) {
      ...
      var params_array =
          [ 'algos', 'caseids', 'metric', 'input', 'output', 'result_exe_id' ]
        ...
        .insertOne({
          ...
          "status" : status
                   ...
                   function(err, result) {
                     ...
                     // console.log("Inserted a document into the restaurants
                     // collection.");
                     db.close();
                   });
...
var server = app.listen(8127,
                 ...
                 var insertStatusByJobId =
    function(db, status, jobid, callback) {
  db.collection(COLLECTION2)
      .insertOne({
        "job_id" : jobid,
        "status" : status
