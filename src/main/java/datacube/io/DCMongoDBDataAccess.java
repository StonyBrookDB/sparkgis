                ...
                if (option == 0) {
                        ...
                        for (final DBObject result : minMax.results()) {
                                for (DCDimension dim : dimensions) {
                                        ...
                                        String tmp_key_min =dim.getNameStr()  + "_min";
                                        // System.out.println("tmp_key_min:  "+tmp_key_min);
                                        String tmp_key_max =dim.getNameStr()  + "_max";
                                        if (result.containsKey(tmp_key_min))
                                                dim.setMin((Double)result.get(tmp_key_min));
                                        if (result.containsKey(tmp_key_max))
                                                dim.setMax((Double)result.get(tmp_key_max));
                                }
...
private void insertMetaData(
        ...
        try{
                ...
                while(iterator.hasNext()) {
                        ...
                        //if (entry.getKey().equals("db") || entry.getKey().equals("collection"))
                        //    continue;
                        String key = ((String)entry.getKey()).replaceAll("\\.", "_range_");
                        Object value = entry.getValue();
                        if (value instanceof String) {
                                metaData.append(key,(String)value);
                        }
